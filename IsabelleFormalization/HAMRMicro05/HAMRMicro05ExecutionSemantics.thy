...
inductive systemStep for sys::"'value System"
                where
 stepTask: "\<lbrakk>
         \<comment> \<open>Non-deterministically pick a fireable transition or {|END|} if in (ready sysState), choice\<close>
         choice \<in> {s. s |\<subseteq>| (ready sysState) \<and> (s \<in> (dom (nextRel sys)) \<or> s = {|modelEndRid m|})}; 
         ...
         ready' = scheduleNextTotal (nextRel sys) m (ready sysState) choice;
         ...
         \<Longrightarrow> systemStep sys m sysState sysState'"
| stepSkip: "\<lbrakk>
         \<comment> \<open>Non-deterministically pick a fireable transition or {|END|} if in (ready sysState), choice\<close>
         choice \<in> {s. s |\<subseteq>| (ready sysState) \<and> (s \<in> (dom (nextRel sys)) \<or> s = {|modelEndRid m|})}; 
...
lemma stepCurrentStorePreviousStore:
  assumes step: "systemStep sys m st1 st2"
...
definition systemReach :: "'store System \<Rightarrow> Model \<Rightarrow> 'store SystemState \<Rightarrow> bool" where 
 "systemReach sys m st = ((systemStep sys m)\<^sup>*\<^sup>* (initSystemState sys) st)"
...
proof (cases)
      ...
      proof (rule impI)
        ...
        proof (elim disjE)
          ...
          from this r4 r5  have "wf_TaskState ((modelTaskDescrs m) $ tida) (fst ((action ((taskMap sys) $ tida)) ((the (systemTasks st1 tida)), (systemChs st1))))"
            by (metis wf_System_TaskMap_def wf_System_def wf_sys(1))
  ...
  from this a3 a7 have reachable_st2: "wf_SystemState_Reach sys m st2"
    apply (simp add: wf_SystemState_Reach_def mkSystemState_def)
    apply (rule conjI)
     apply (rule impI)
    apply (simp add: scheduleReachBody_def)
  proof 
    assume a11: "scheduleReachBody (nextRel sys) {|modelStartRid m|} (ready st1)"
    assume a12: "ready' = (if choice = {|modelEndRid m|} 
                           then {|modelStartRid m|} 
                           else scheduleNextBody (nextRel sys) (ready st1) choice)"
    assume a13: "st2 = \<lparr>systemTasks = (systemTasks st1)(tid \<mapsto> fst tscs'), 
                        systemChs = snd tscs', 
                        ready = if choice = {|modelEndRid m|} 
                                then {|modelStartRid m|} 
                                else scheduleNextBody (nextRel sys) (ready st1) choice, 
                        previousSystemTasks = systemTasks st1,
                        previousSystemChs = systemChs st1\<rparr>"
    assume a14: "choice \<noteq> {|modelEndRid m|}"

    \<comment> \<open>the choice of move should be in the set of available actions\<close>
    have r1: "choice \<in> {s. s |\<subseteq>| ready st1 \<and> (s \<in> dom (nextRel sys) \<or> s = {|modelEndRid m|})}"
      using a1 by auto
               
    \<comment> \<open>the schedule state of st2 should be the result of updating the schedule state of st1
        with choice\<close>
    from a12 a13 a14 have r3: "ready st1 |-| choice |\<union>| (nextRel sys $ choice) = ready st2"
      by simp
    \<comment> \<open>because the update is possible, there should be a schedule step from (ready st1) to (ready st2)\<close>
    from r1 a14 r3 have "scheduleStepBody (nextRel sys) (ready st1) (ready st2)"
      apply (simp add: scheduleStepBody_def)
      by auto
    ...
    from this a11 have "scheduleReachBody (nextRel sys) {|modelStartRid m|} (ready st2)"
      by (simp add: scheduleReachBody_def)
    \<comment> \<open>we can state an equivalent statement using the schedule state update described by st2\<close>
    from this r3 show "scheduleReachBody (nextRel sys) {|modelStartRid m|} (ready st1 |-| choice |\<union>| the (nextRel sys choice))"
      by auto
  qed

  \<comment> \<open>No Deadlock\<close>        

  \<comment> \<open>the schedule state of st1 conditionally does not deadlock in the NextRel (not overall)\<close>
  ...
  from wf_SystemState_NoDeadlock_st1 a3 a7 NoDeadlockAndOptToCompl have wf_SystemState_NoDeadlock_st2: "wf_SystemState_ConditionalDeadlock sys m st2"
    ...
    proof (erule conjE)
      ...
      assume a12: "ready' = scheduleNextTotal (nextRel sys) m (ready st1) choice"
      assume a13: "st2 = \<lparr>systemTasks = (systemTasks st1)(tid \<mapsto> fst tscs'), 
                          systemChs = snd tscs',
                          ready = ready', 
                          previousSystemTasks = systemTasks st1, 
      ...
      proof auto
        ...
        proof (rule ccontr)
          ...
          from this have reach: "scheduleReachBody (nextRel sys) (ready st2) {|modelEndRid m|}"
          ...
          have "\<forall>schst1 schst2. (schst1 \<noteq> schst2 \<and> scheduleReachBody (nextRel sys) schst1 schst2 
                 \<longrightarrow> (\<exists>schst3. schst1 \<noteq> schst3 \<and> scheduleStepBody (nextRel sys) schst1 schst3))"
            by (smt (verit, del_insts) converse_rtranclpE rtranclp_induct scheduleReachBody_def)
          ...
          from this reach noteq have "\<exists>schst. schst \<noteq> (ready st2) \<and> scheduleStepBody (nextRel sys) (ready st2) schst"
            apply (simp add: scheduleReachBody_def scheduleStepBody_def)
            by metis
          ...
          then have "\<exists> t. t \<in> {s. s |\<subseteq>| (ready st2) \<and> s \<in> (dom (nextRel sys))}"
            using scheduleStepBody_def by auto
      ...
      next
        ...
        thus  False
          by (metis (no_types, lifting) a112 card.empty domIff empty_Collect_eq fempty_fsubsetI 
                    fsubset_fsingletonD nextAssms nextRel_Assumptions_def order_less_imp_not_less 
                    wf_System_NextRel_TransitionOnPath_def)
          
      next
        \<comment> \<open>if there are no available moves in st2 the END RID must be in the schedule state of st2\<close>
        ...
        proof (rule ccontr)
          ...
          \<comment> \<open>because of this, there must exist a schst not equal to the schedule state of st2 such
              that the schedule can step from the schedule state of st2 to schst\<close>
          from this reach noteq have "\<exists>schedst. schedst \<noteq> (ready st2) \<and> scheduleStepBody (nextRel sys) (ready st2) schedst"
            apply (simp add: scheduleReachBody_def scheduleStepBody_def)
            by metis
    ...
    from this a1 a3 a7 wf_model have wf_SystemState_ReadySub_st2: "wf_SystemState_ReadySub sys m st2" 
      apply (simp add: wf_SystemState_ReadySub_def mkSystemState_def)
      apply (rule conjI)
       apply (simp add: wf_Model_Rids_def wf_Model_def)
      apply (erule conjE)
      apply clarify      
    proof
      fix y
      assume r1subRIDS: "ready st1 |\<subseteq>| modelReadyIds m"
      assume rUpdate: "ready' = (if choice = {|modelEndRid m|} 
                                 then {|modelStartRid m|} 
                                 else scheduleNextBody (nextRel sys) (ready st1) choice)"
      ...
      show "ready st1 |-| choice |\<subseteq>| modelReadyIds m"
    ...
    next
      ...
      \<comment> \<open>the set of out places of choice are a subset of the model rids\<close>
      thus "the (nextRel sys choice) |\<subseteq>| modelReadyIds m"
        by (simp add: yMove)
    qed
...
next
  ...
  proof 
    ...
    \<comment> \<open>the schedule state of st2 should be the result of updating the schedule state of st1
        with choice\<close>
    from a12 a13 a14 have r3: "ready st1 |-| choice |\<union>| the (nextRel sys choice) = ready st2"
      by simp
    \<comment> \<open>because the move exist, there should be a schedule step from st1 to st2\<close>
    from r1 a14 r3 have "scheduleStepBody (nextRel sys) (ready st1) (ready st2)"
      apply (simp add: scheduleStepBody_def)
      by auto
    \<comment> \<open>because the schedule state of st1 is reachable and the schedule can go from st1 to st2,
        the schedule state of st2 is reachable\<close>
    from this a11 have "scheduleReachBody (nextRel sys) {|modelStartRid m|} (ready st2)"
      by (simp add: scheduleReachBody_def)
    \<comment> \<open>we can state an equivalent statement using the schedule state update described by st2\<close>
  ...
  \<comment> \<open>the schedule state of the schedule state of st1 conditionally does not deadlock in the NextRel
      but not overall\<close>
  from wf_st1 have wf_SystemState_NoDeadlock_st1: "wf_SystemState_ConditionalDeadlock sys m st1" 
    by (simp add: wf_SystemState_def)
  ...
  from wf_SystemState_NoDeadlock_st1 a3 a4 NoDeadlockAndOptToCompl have wf_SystemState_NoDeadlock_st2: "wf_SystemState_ConditionalDeadlock sys m st2"
    ...
    proof (erule conjE)
      ...
      next
        ...
        thus False
          by (metis (mono_tags, lifting) a112 card.empty domIff empty_Collect_eq fempty_fsubsetI 
                                         fsubset_fsingletonD less_imp_neq nextAssms 
                                         nextRel_Assumptions_def wf_System_NextRel_TransitionOnPath_def)
  ...
  \<comment> \<open>wf_SystemState_ReadySub\<close>
    ...
    from this a1 a3 a4 wf_model have wf_SystemState_ReadySub_st2: "wf_SystemState_ReadySub sys m st2" 
      apply (simp add: wf_SystemState_ReadySub_def mkSystemState_def)
      apply (rule conjI)
      apply (rule impI)
       apply (simp add: wf_Model_Rids_def wf_Model_def)
