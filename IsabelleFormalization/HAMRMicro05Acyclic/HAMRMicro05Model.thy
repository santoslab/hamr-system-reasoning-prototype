...
text \<open>Calculates the next ready set given a transition (defined by its input places) by removing 
      ...
      out places of the transition.\<close>
fun scheduleNext :: "Next \<Rightarrow> Ready \<Rightarrow> Rid fset \<Rightarrow> Ready"
  where "scheduleNext n r rf = r - (rf :: nat fset) |\<union>| n $ rf"
...
definition scheduleStep :: "Next \<Rightarrow> Ready \<Rightarrow> Ready \<Rightarrow> bool"
  where "scheduleStep next r1 r2 \<equiv> \<exists> t. t \<in> {s. s |\<subseteq>| r1 \<and> s \<in> (dom next)} 
                                        \<and> (scheduleNext next r1 t = r2)"

definition scheduleReach :: "Next \<Rightarrow> Ready \<Rightarrow> Ready \<Rightarrow> bool" where 
 "scheduleReach next init r = ((scheduleStep next)\<^sup>*\<^sup>* init r)"

definition scheduleReachSet :: "Next \<Rightarrow> Ready \<Rightarrow> Rid set"
  where "scheduleReachSet next readySet \<equiv> {r. \<exists>rset. scheduleReach next readySet rset \<and> r |\<in>| rset}"
...
definition wf_System_NextRel_PlaceOnPath :: "'value System \<Rightarrow> Model \<Rightarrow> bool"
  where "wf_System_NextRel_PlaceOnPath sys m \<equiv>
          \<forall>rid \<in> fset (modelReadyIds m). 
            \<exists>M M'. scheduleReach (nextRel sys) {|modelStartRid m|} M 
                \<and> rid |\<in>| M
                \<and> rid |\<notin>| M'
                \<and> scheduleStep (nextRel sys) M M'
                \<and> scheduleReach (nextRel sys) M' {|modelEndRid m|}"
...
definition wf_System_NextRel_OptToComplete :: "'value System \<Rightarrow> Model \<Rightarrow> bool"
  where "wf_System_NextRel_OptToComplete sys m \<equiv> 
          \<forall>mark. scheduleReach (nextRel sys) {|modelStartRid m|} mark 
                 \<longrightarrow> scheduleReach (nextRel sys) mark {|modelEndRid m|}"
...
definition wf_System_NextRel_ProperComplete :: "'value System \<Rightarrow> Model \<Rightarrow> bool"
  where "wf_System_NextRel_ProperComplete sys m \<equiv>  
          \<forall>mark. (scheduleReach (nextRel sys) {|modelStartRid m|} mark \<and> modelEndRid m |\<in>| mark)
...
definition wf_System_NextRel_NoDeadTransitions :: "'value System \<Rightarrow> Model \<Rightarrow> bool"
  where "wf_System_NextRel_NoDeadTransitions sys m \<equiv> 
          \<forall>t \<in> (dom (nextRel sys)). 
            \<exists>M. scheduleReach (nextRel sys) {|modelStartRid m|} M \<and> t |\<subseteq>| M"
...
definition wf_System_NoCycle :: "'value System \<Rightarrow> Model \<Rightarrow> bool"
  where "wf_System_NoCycle sys m \<equiv>
          \<forall>schst. scheduleReach (nextRel sys) {|modelStartRid m|} schst
                  \<longrightarrow>
...
text \<open>Every task in the task map should have a task state\<close>
...
definition wf_SystemState_Reach :: "'a System \<Rightarrow> Model \<Rightarrow> 'a SystemState \<Rightarrow> bool"
  where "wf_SystemState_Reach sys m ss \<equiv> scheduleReach (nextRel sys) {|modelStartRid m|} (ready ss)"
