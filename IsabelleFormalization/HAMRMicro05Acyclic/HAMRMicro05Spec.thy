...
definition MHIP_assumption :: "MayHappenInParallelRel \<Rightarrow> 'value System \<Rightarrow> Model \<Rightarrow> bool"
  where "MHIP_assumption mhip sys m \<equiv> \<forall>f s. f \<in> dom(nextRel sys) \<and> s \<in> dom(nextRel sys) \<and> f \<noteq> s \<longrightarrow>
          (f, s) \<in> mhip 
          \<longleftrightarrow> 
          (\<exists>schst. scheduleReach (nextRel sys) {|modelStartRid m|} schst \<and> f |\<subseteq>| schst \<and> s |\<subseteq>| schst)"
...
definition BackStepReachability_Assumption :: "'value System \<Rightarrow> Model \<Rightarrow> bool"
  where "BackStepReachability_Assumption sys m \<equiv> 
          \<forall>f s. f \<in> dom(nextRel sys) 
                ...
                \<longrightarrow>
                (\<exists>schst. scheduleReach (nextRel sys) {|modelStartRid m|} schst \<and> f |\<subseteq>| schst \<and> (nextRel sys $ s) |\<subseteq>| schst) 
                \<longrightarrow> 
                (\<exists>schst. scheduleReach (nextRel sys) {|modelStartRid m|} schst \<and> f |\<subseteq>| schst \<and> s |\<subseteq>| schst)
            "
...
meant to act as an auxiliary proof so that the system level proofs do not have to refer to the action
...
text \<open>The pre-assertions for a component transition should imply the pre-condition of the component\<close>
...
(* ONLY USED IN CYCLIC VERSION*)

(*
definition prePostAssertVC :: "'value System \<Rightarrow> Model \<Rightarrow> 'value SystemSpec \<Rightarrow> bool"
  where "prePostAssertVC sys m sysSpec \<equiv> 
            \<forall>(st :: 'value SystemState). 
...
proof (cases)
  ...
  from a7 a3 have "((nextRel sys) $ choice) |\<subseteq>| (ready st2)"
  ...
  have altChoiceHold: "\<forall>c \<in> {s. s |\<subseteq>| ready st1 - choice \<and> s \<in> dom (nextRel sys)}. ballAssert sysSpec st2 c"
  proof
    fix c
    assume cIsChoice: "c \<in> {s. s |\<subseteq>| ready st1 |-| choice \<and> s \<in> dom (nextRel sys)}"
    ...
    from choiceNotEmpty cIsChoice have "c \<noteq> choice"
      by auto

    \<comment> \<open>choice and c may happen in parallel\<close>
    from this mhipAssm wf_st1 a1 cIsChoice have "(choice, c) \<in> mhip"
  ...
  from this wf_sys have "finite {s. s |\<subseteq>| ready st1 \<and> s \<in> dom (nextRel sys)}"
    by simp
  ...
  have ete: "\<forall>p \<in> fset (ready st1 - choice) - (\<Union> (fset`{s. s |\<subseteq>| ready st1 - choice \<and> s \<in> dom (nextRel sys)})).
        ((sysPlaceAsserts sysSpec) $ p) (systemTasks st2) (systemChs st2)"
  proof 
    fix p
    assume pLeft: "p \<in> fset (ready st1 |-| choice) - \<Union> (fset ` {s. s |\<subseteq>| ready st1 |-| choice \<and> s \<in> dom (nextRel sys)})"
    ...
    have pNotC: "\<forall>c \<in> {s. s |\<subseteq>| ready st1 - choice \<and> s \<in> dom (nextRel sys)}. p |\<notin>| c" using pLeft by auto 
    ...
    from this pReady pNotChoice pNotC have  "c \<noteq> {|p|}"
      by blast
    ...
    from wf_st1 c'Dom c'Res pReady nextAssms a1 have notEqC': "c' \<noteq> choice"
      apply (auto simp add: nextRel_Assumptions_def wf_System_NoCycle_def wf_SystemState_def wf_SystemState_Reach_def)
    ...
    from obtainC sizeC existC' origOfP 
    have cIsEndPath: "c \<in> dom (nextRel sys) \<and> fcard (c) > 1 \<and> (nextRel sys $ c') |\<subseteq>| c"
      by simp
    \<comment> \<open>therefore there exist a transition that satisfies the following constraints\<close>
    from this have existEndPath: "\<exists>d \<in> dom (nextRel sys). fcard (d) > 1 \<and> (nextRel sys $ c') |\<subseteq>| d"
      by auto  
    ...
    have "scheduleReach (nextRel sys) {|modelStartRid m|} (ready st1) \<and> choice |\<subseteq>| (ready st1) \<and> the (nextRel sys c') |\<subseteq>| (ready st1)"
    ...
    from this existEndPath notEqC' bstrAssm pReady a1 c'Res c'Dom notEqC cNotEqC'
    have choicec'Reach: "(\<exists>schst. scheduleReach (nextRel sys) {|modelStartRid m|} schst \<and> choice |\<subseteq>| schst \<and> c' |\<subseteq>| schst)"
      apply (simp add: BackStepReachability_Assumption_def)
      by (metis One_nat_def cIsEndPath subsetc')
    \<comment> \<open>therefore choice and c' mhip\<close>
    from this mhipAssm a1 c'Dom notEqC' have cc'MHIP: "(choice, c') \<in> mhip"
    ...
    proof-
      ...
      from this allPlaceAssert pReady 
      have sysSpecC: "(sysPlaceAsserts sysSpec $ p) (systemTasks st1) (systemChs st1)"
        using ballAssert_def by blast
  ...
  from a1 a2 a3 a7 have r2: "fset (ready st2) = fset (ready st1 - choice) \<union> fset ((nextRel sys) $ choice)"
  ...
  from altChoiceHold have expandACH: "\<forall>p \<in> \<Union> (fset`{s. s |\<subseteq>| ready st1 - choice \<and> s \<in> dom (nextRel sys)}). 
                            ((sysPlaceAsserts sysSpec) $ p) (systemTasks st2) (systemChs st2)"
    by (auto simp add: ballAssert_def)
  ...
  have "fset (ready st1 - choice) = 
           (\<Union> (fset`{s. s |\<subseteq>| ready st1 - choice \<and> s \<in> dom (nextRel sys)})) 
           \<union>
           (fset (ready st1 - choice) - (\<Union> (fset`{s. s |\<subseteq>| ready st1 - choice \<and> s \<in> dom (nextRel sys)})))"
    by blast
...
next
  ...
  \<comment> \<open>The out places of choice are in the schedule state of st2\<close>
  from a4 a3 have "((nextRel sys) $ choice) |\<subseteq>| (ready st2)"
    by (auto simp add: mkSystemState_def)
  \<comment> \<open>All post assertions of choice assertions are true for st2\<close>
  ...
  proof -
    ...
    \<comment> \<open>Because systemNextAssertVC is true and the pre-assertions hold, all post-assertions that are 
        the direct result of the choice are true\<close>
  ...
  \<comment> \<open>all assertions tied to a place not in choice should hold for st2\<close>
  ...
  \<comment> \<open>ready st2 is the result of removing the in places of choice from ready st1 and adding the 
      out places of choice\<close>
  from a1 a2 a3 a4 have "ready st2 = ready st1 - choice |\<union>| (nextRel sys) $ choice"
    by (simp add: mkSystemState_def)

  \<comment> \<open>because ready st2 consists of all places remaining from st1 and the result of stepping the
      schedule and all assertions tied to both sets hold for st2, all assertions tied to the 
      schedule state of st2 hold for st2\<close>
