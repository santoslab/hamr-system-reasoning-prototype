...
@sig trait Manage_Heat_Source_i_Api {
  def id: Art.BridgeId
...
@datatype class Manage_Heat_Source_i_Initialization_Api (
  val id: Art.BridgeId,
  ...
  val heat_control_Id : Art.PortId) extends Manage_Heat_Source_i_Api

@datatype class Manage_Heat_Source_i_Operational_Api (
  val id: Art.BridgeId,
  val current_tempWstatus_Id : Art.PortId,
  val lower_desired_temp_Id : Art.PortId,
  val upper_desired_temp_Id : Art.PortId,
  ...
  def get_lower_desired_temp() : Option[Isolette_Data_Model.Temp_i] = {
    ...
    val value : Option[Isolette_Data_Model.Temp_i] = Art.getValue(lower_desired_temp_Id) match {
      case Some(Isolette_Data_Model.Temp_i_Payload(v)) => Some(v)
      case Some(v) =>
        Art.logError(id, s"Unexpected payload on port lower_desired_temp.  Expecting 'Isolette_Data_Model.Temp_i_Payload' but received ${v}")
  ...
  def get_upper_desired_temp() : Option[Isolette_Data_Model.Temp_i] = {
    ...
    val value : Option[Isolette_Data_Model.Temp_i] = Art.getValue(upper_desired_temp_Id) match {
      case Some(Isolette_Data_Model.Temp_i_Payload(v)) => Some(v)
      case Some(v) =>
        Art.logError(id, s"Unexpected payload on port upper_desired_temp.  Expecting 'Isolette_Data_Model.Temp_i_Payload' but received ${v}")
        None[Isolette_Data_Model.Temp_i]()
      case _ => None[Isolette_Data_Model.Temp_i]()
    }
    return value
