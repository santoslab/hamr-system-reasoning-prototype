...
@msig trait Manage_Regulator_Interface_i_thermostat_rt_mri_mri_GumboX_TestHarness extends Manage_Regulator_Interface_i_thermostat_rt_mri_mri_TestApi {
  def verbose: B
  ...
  def testInitialiseCB(
    ...
    val api_displayed_temp: Isolette_Data_Model.Temp_i = get_displayed_temp().get
    val api_interface_failure: Isolette_Data_Model.Failure_Flag_i = get_interface_failure().get
    val api_lower_desired_temp: Isolette_Data_Model.Temp_i = get_lower_desired_temp().get
    val api_regulator_status: Isolette_Data_Model.Status.Type = get_regulator_status().get
    val api_upper_desired_temp: Isolette_Data_Model.Temp_i = get_upper_desired_temp().get
    ...
    val postResult = isolette.Regulate.Manage_Regulator_Interface_i_thermostat_rt_mri_mri_GumboX.inititialize_IEP_Post(api_displayed_temp, api_interface_failure, api_lower_desired_temp, api_regulator_status, api_upper_desired_temp)
  ...
  def testComputeCBJ(json: String): GumboXResult.Type = {
    isolette.JSON.toRegulateManage_Regulator_Interface_i_thermostat_rt_mri_mri_PreState_Container(json) match {
      case Either.Left(o) => return testComputeCBV(o)
      case Either.Right(msg) => halt(msg.string)
    }
  }

  def testComputeCBV(o: Manage_Regulator_Interface_i_thermostat_rt_mri_mri_PreState_Container): GumboXResult.Type = {
  ...
  def testComputeCB(
      ...
      api_upper_desired_tempWstatus: Isolette_Data_Model.TempWstatus_i): GumboXResult.Type = {
    ...
    //   mri does not have incoming ports or state variables

    // [CheckPre]: check/filter based on pre-condition.
    val CEP_Pre_Result: B = isolette.Regulate.Manage_Regulator_Interface_i_thermostat_rt_mri_mri_GumboX.compute_CEP_Pre (api_current_tempWstatus, api_lower_desired_tempWstatus, api_regulator_mode, api_upper_desired_tempWstatus)
    if (!CEP_Pre_Result) {
    ...
    // [CheckPost]: invoke the oracle function
    val postResult = isolette.Regulate.Manage_Regulator_Interface_i_thermostat_rt_mri_mri_GumboX.compute_CEP_Post(api_current_tempWstatus, api_lower_desired_tempWstatus, api_regulator_mode, api_upper_desired_tempWstatus, api_displayed_temp, api_interface_failure, api_lower_desired_temp, api_regulator_status, api_upper_desired_temp)
    val result: GumboXResult.Type =
      if (!postResult) GumboXResult.Post_Condition_Fail
      else GumboXResult.Post_Condition_Pass
